...
firebase.database().ref("messages").on("child_added", function(snapshot) {
  ...
  html += `<span id='message-date' data-date='${snapshot.val().time}'></span>`;
...
var unread = null;
window.onblur = function() { unread = document.querySelectorAll("li").length; };
window.onfocus =
    function() { unread = document.querySelectorAll("li").length; };
setInterval(() => {
  updatedates();
  timer = timer + 50;
  if (myName != null) {
  ...
  if (document.querySelectorAll("li").length > unread) {
    document.title =
        `(${document.querySelectorAll("li").length - unread}) Chat App`;
  } else {
...
document.addEventListener(
    "mousemove", (e) => { unread = document.querySelectorAll("li").length; });
document.addEventListener(
    "mousedown", (e) => { unread = document.querySelectorAll("li").length; });
document.addEventListener(
    "keydown", (e) => { unread = document.querySelectorAll("li").length; });
...
function playsound(
    url =
        "https://proxy.notificationsounds.com/notification-sounds/me-too-603/download/file-sounds-1144-me-too.mp3") {
